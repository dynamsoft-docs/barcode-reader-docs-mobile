---
layout: default-layout
title: Index of Dynamsoft Capture Vision Flutter edition
description: This page is the index of Dynamsoft Capture Vision - Flutter edition
keywords: index, capture vision
needAutoGenerateSidebar: true
needGenerateH3Content: true
noTitleIndex: true
breadcrumbText: Flutter Index
---

# Documentation - Flutter

## Getting Started

- [Getting Started with Barcode Reader Module]({{site.flutter}}user-guide/barcode-reader.html)

## Samples

- <a href = "https://github.com/Dynamsoft/capture-vision-flutter-samples" target="_blank">Dynamsoft Capture Vision - Flutter barcode_reader_simple_sample</a>

## API Reference

- [DCVCameraView class]({{site.flutter_api}}camera-view.md)
- [DCVCameraEnhancer class]({{site.flutter_api}}camera-enhancer.md)
- [DCVBarcodeReader class]({{site.flutter_api}}barcode-reader.md)

## Release Notes

- [Version 1.x]({{site.flutter}}release-notes/flutter-1.md)
