...
description: The introduction of Dynamsoft Capture Vision Architecture for iOS.
...
`DSImageSourceAdapter` is the standard input for Capture Vision architecture. Once the capture process starts, CVR continuously acquires image data from the `DSImageSourceAdapter` until the capture process is stopped or the image source is exhausted.

You can directly use the implementations provided by Dynamsoft:

- Camera: `DSCameraEnhancer`
- File/Directory: `DSDirectoryFetcher`

You can also create a custom `DSImageSourceAdapter`.
...
`DSCapturedResult` is the standard output of Dynamsoft Capture Vision. It contains all results generated during image processing, including barcode results, parsed results, and other captured data.

`DSBarcodeResultItem` represents a single detected barcode and contains the complete information for that barcode. If multiple barcodes are recognized in a single scan, the barcode result will contain multiple `DSBarcodeResultItem` objects.
...
From the beginning of image processing to the generation of a `DSCapturedResult`, the algorithm goes through multiple stages. The output produced at each stage is called an intermediate result.
