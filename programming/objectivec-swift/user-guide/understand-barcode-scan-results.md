...
- `finished`: Scanning completed successfully. At least one barcode is available.
- `canceled` / `exception`: No barcodes are returned.

`exception` might be caused by:

- You configured an invalid or expired license via `BarcodeScannerConfig.license`.
- You configured an invalid template via `BarcodeScannerConfig.templateFile`.
